...
typedef struct my_fc_fwd_config {
  ...
  libxsmm_smmfunction_reducebatch_strd_meltwfused gemm_fwd3;
  libxsmm_smmfunction_reducebatch_strd_meltwfused gemm_fwd4;
  libxsmm_smmfunction_reducebatch_strd_meltwfused gemm_fwd5;
  libxsmm_smmfunction_reducebatch_strd_meltwfused gemm_fwd6;
...
my_fc_fwd_config setup_my_fc_fwd(libxsmm_blasint N, libxsmm_blasint C, libxsmm_blasint K, libxsmm_blasint bn,
  ...
  res.gemm_fwd3 = libxsmm_smmdispatch_reducebatch_strd_meltwfused(res.bk, res.bn, res.bc, res.bk*res.bc*sizeof(float), res.bc*res.bn*sizeof(float), &lda, &ldb, &ldc, &alpha, &zerobeta, &l_flags, NULL, LIBXSMM_MELTW_OPERATION_COLBIAS_ACT, LIBXSMM_DATATYPE_F32, fusion_flags, 0, 0, 0, 0);
  if ( res.gemm_fwd3 == NULL ) {
    fprintf( stderr, "JIT for BRGEMM TPP gemm_fwd3 failed. Bailing...!\n");
    exit(-1);
  }
#ifdef USE_RELU_BITMASK
  fusion_flags = LIBXSMM_MELTW_FLAG_ACT_RELU_OVERWRITE_C;
#else
  fusion_flags = LIBXSMM_MELTW_FLAG_ACT_RELU_NOBITMASK_OVERWRITE_C;
#endif
  res.gemm_fwd4 = libxsmm_smmdispatch_reducebatch_strd_meltwfused(res.bk, res.bn, res.bc, res.bk*res.bc*sizeof(float), res.bc*res.bn*sizeof(float), &lda, &ldb, &ldc, &alpha, &zerobeta, &l_flags, NULL, LIBXSMM_MELTW_OPERATION_COLBIAS_ACT, LIBXSMM_DATATYPE_F32, fusion_flags, 0, 0, 0, 0);
...
#else
  fusion_flags = LIBXSMM_MELTW_FLAG_COLBIAS_ACT_RELU_NOBITMASK_OVERWRITE_C;
#endif
  res.gemm_fwd5 = libxsmm_smmdispatch_reducebatch_strd_meltwfused(res.bk, res.bn, res.bc, res.bk*res.bc*sizeof(float), res.bc*res.bn*sizeof(float), &lda, &ldb, &ldc, &alpha, &zerobeta, &l_flags, NULL, LIBXSMM_MELTW_OPERATION_COLBIAS_ACT, LIBXSMM_DATATYPE_F32, fusion_flags, 0, 0, 0, 0);
...
void my_fc_fwd_exec( my_fc_fwd_config cfg, const float* wt_ptr, const float* in_act_ptr, float* out_act_ptr,
  ...
  libxsmm_smmfunction_reducebatch_strd_meltwfused batchreduce_kernel_zerobeta_fused_eltwise;
  ...
  if (((cfg.fuse_type & MY_ELTWISE_FUSE_BIAS) == MY_ELTWISE_FUSE_BIAS) && ((cfg.fuse_type & MY_ELTWISE_FUSE_RELU) == MY_ELTWISE_FUSE_RELU )) {
    batchreduce_kernel_zerobeta_fused_eltwise = cfg.gemm_fwd5;
  } else if ((cfg.fuse_type & MY_ELTWISE_FUSE_BIAS) == MY_ELTWISE_FUSE_BIAS) {
    batchreduce_kernel_zerobeta_fused_eltwise = cfg.gemm_fwd3;
  } else if ((cfg.fuse_type & MY_ELTWISE_FUSE_RELU) == MY_ELTWISE_FUSE_RELU) {
    batchreduce_kernel_zerobeta_fused_eltwise = cfg.gemm_fwd4;
  } else {
    batchreduce_kernel_zerobeta_fused_eltwise = NULL;
  }
    ...
    if (BF > 1) {
      ...
      for (ofm1 = my_M_start; ofm1 < my_M_end; ++ofm1) {
        for (mb1 = my_N_start; mb1 < my_N_end; ++mb1) {
          if ( ((cfg.fuse_type & MY_ELTWISE_FUSE_BIAS) == MY_ELTWISE_FUSE_BIAS) || ((cfg.fuse_type & MY_ELTWISE_FUSE_RELU) == MY_ELTWISE_FUSE_RELU )) {
            if ((cfg.fuse_type & MY_ELTWISE_FUSE_BIAS) == MY_ELTWISE_FUSE_BIAS) {
              gemm_eltwise_params.bias_ptr  = (void*) &LIBXSMM_VLA_ACCESS(2, bias, ofm1, 0,cfg.bk);
            }
#ifdef USE_RELU_BITMASK
            if ((cfg.fuse_type & MY_ELTWISE_FUSE_RELU) == MY_ELTWISE_FUSE_RELU) {
              gemm_eltwise_params.out_ptr = (void*) &LIBXSMM_VLA_ACCESS(4, relubitmask, mb1, ofm1, 0, 0, nBlocksOFm, cfg.bn, cfg.bk/8);
            }
#endif
            batchreduce_kernel_zerobeta_fused_eltwise( &LIBXSMM_VLA_ACCESS(4, filter, ofm1, 0, 0, 0, nBlocksIFm, cfg.bc, cfg.bk),
                &LIBXSMM_VLA_ACCESS(4, input,  mb1, 0, 0, 0, nBlocksIFm, cfg.bn, cfg.bc),
                &LIBXSMM_VLA_ACCESS(4, output, mb1, ofm1, 0, 0, nBlocksOFm, cfg.bn, cfg.bk), &blocks, &gemm_eltwise_params);
          } else {
